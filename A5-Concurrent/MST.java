...
public class MST {
    ...
    private MSTworld build(RootPaneContainer pane, int an) {
        ...
        MSTworld w = new MSTworld(n, sd, c, numThreads);
        Animation t = null;
    ...
    public static void main(String[] args) {
        ...
        if (animate == SHOW_RESULT || animate == FULL_ANIMATION) {
            f = new JFrame("MST-Extra-credit-2");
...
class MSTworld {
    ...
    // Constructor
    //
    public MSTworld(int N, long SD, Coordinator C, int threads) {
        n = N;
        sd = SD;
        coord = C;
        numThreads = threads;
    ...
    public void DwyerSolve() throws Coordinator.KilledException {
    	...
    	forkJoinPool.invoke(new Triangulate(0, n-1, minx, maxx, miny, maxy, 0));
    	
    	forkJoinPool.shutdown();
    }
    
    public class Triangulate extends RecursiveAction {
//      private void triangulate(int l, int r, int low0, int high0,
//                               int low1, int high1, int parity) 
//          throws Coordinator.KilledException {
      	int l; int r; int low0; int high0;
          int low1; int high1; int parity;
          
      	public Triangulate(int l, int r, int low0, int high0,
      	        int low1, int high1, int parity) {
              this.l = l;
              this.r = r;
              this.low0 = low0;
      	...
      	private void triangulate(int l, int r, int low0, int high0,
                int low1, int high1, int parity)throws Coordinator.KilledException {
			...
			int mid = low0/2 + high0/2; // mid of x coordination
			int i = l;  int j = r;
			...
			while (i < j) {
			   i0 = points[i].getCoord(dim0);//get x coordinate
          ...
          protected void compute(){
  	        ...
  	        if (l == r-2) {     // make single triangle
  	            edge e2 = null;
  				try {
  					e2 = new edge(points[l+1], points[r], null, null, dir1);
  				} catch (Coordinator.KilledException e) {
  					e.printStackTrace();
  				}
  	            ...
  	            edge e1 = null;
  				try {
  					e1 = new edge(points[l], points[l+1], null, e2, dir1);
  				} catch (Coordinator.KilledException e) {
  					e.printStackTrace();
  				}
  	        ...
  	        //System.out.println("printI: "+i+", "+ "printJ: "+j);
  	        ...
  	        while (true) {
  	            ...
  	            while (i < j) {
  	                i0 = points[i].getCoord(dim0);//get x
  	                if (i0 > mid) {     // belongs in right half
  	                    if (i0 < rp0) {
  	                        rp0 = i0;  rp = points[i];
  	                    }
  	                	...
  	                	// i0 <= mid
  	                    if (i0 > lp0) {
  	                    	// find a new rightmost point in left half;
  	                        lp0 = i0;  lp = points[i];
  	                    }
  	                }
  	                i++;
  	        ...
  	        if (i < l) {
        		...
        		try {
					triangulate(j, r, low1, high1, mid, high0, 1-parity);
				} catch (Coordinator.KilledException e) {e.printStackTrace();}
  	        	
  	        	
  	        } else if (j > r) {
  	            // empty right half
  	        	try {
					triangulate(l, i, low1, high1, mid, high0, 1-parity);
				} catch (Coordinator.KilledException e) {e.printStackTrace();}
  	            ...
  	            // divide and conquer
  	        	if (r-j > 20) {
  	        		Triangulate right_triangulate = new  Triangulate(j, r, low1, high1, mid, high0, 1-parity);
  	        		right_triangulate.fork();
  	        		
  	        		if (i-l > 20) {
  	        			Triangulate left_triangulate =  new Triangulate(l, i, low1, high1, low0, mid, 1-parity);
  	        			left_triangulate.fork(); 
  	        			
  	    	        	left_triangulate.join();
  	        		}else {
  	        			try {
  							triangulate(l, i, low1, high1, mid, high0, 1-parity);
  						} catch (Coordinator.KilledException e) {e.printStackTrace();}
  	        		}
  	        		right_triangulate.join();
  	        		
  	        	}else {
  	        		Triangulate left_triangulate = null;
  	        		if (i-l > 20) {
  	        			left_triangulate =  new Triangulate(l, i, low1, high1, low0, mid, 1-parity);
  	        			left_triangulate.fork();
  	        		}else {
  	        			try {
  							triangulate(l, i, low1, high1, mid, high0, 1-parity);
...
//  	        	left_triangulate.join();
  	            ...
  	            while (true) {
  	                ...
  	                point lc = null;
  					try {
  						lc = findCandidate.call(left, dir0, bottom, right.p);
  					} catch (Coordinator.KilledException e) {
  						e.printStackTrace();
  					}
  	                point rc = null;
  					try {
  						rc = findCandidate.call(right, dir1, bottom, left.p);
  					} catch (Coordinator.KilledException e) {
  						e.printStackTrace();
  					}
